...
def get_opts():
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default=os.path.join(dir_path, "silica/silica.ckpt"),
        help="pretrained checkpoint path to load",
    )
    parser.add_argument("--port", type=int, default=8889, help="port to run server")
...
if __name__ == "__main__":
    ...
    def nerf_func(points, dirs):
        ...
        xyzdir_embedded = torch.cat(
            [
                xyz_embedded.expand(-1, dirs.size(1), -1),
                dir_embedded.expand(points.size(0), -1, -1),
            ],
            -1,
        )
